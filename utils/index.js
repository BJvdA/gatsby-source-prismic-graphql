...
function PrismicLink(_ref) {
  ...
  if (BaseURIReg.test(uri)) {
    var prismicClient = _prismicJavascript.default.client(_prismic.Endpoints.v2(uri), {
    ...
    var editButtonLink = new _apolloLink.ApolloLink(function (operation, forward) {
      operation.setContext(function (_ref3) {
        var headers = _ref3.headers;
        var mainContext = operation.getContext().graphqlContext || {};
        var pageContext = mainContext.context || {};
        var pageHeaders = pageContext.headers;
        return {
          headers: (0, _objectSpread2.default)({}, headers, pageHeaders)
        };
      });
    ...
    return _apolloLink.ApolloLink.from([prismicLink, editButtonLink, httpLink]);
  } else {
